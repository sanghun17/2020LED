...
clock_t stime, etime;
	...
	int mark[4][2] = { 0 };
	//int mark[4][2] = { {1126, 424}, {875, 407}, {866 , 646}, {1115, 656} };
	bool far = 0;
	int RESIZE_LED;
	if (far == 1)
		RESIZE_LED = 1;
	...
	else //far == 0
	{
		//resize
		start1 = clock();
		resize(image_input, image_resized, Size(2048*5 , 1536*5 ), INTER_NEAREST);
		end1 = clock();
	...
	end5 = clock();
	cout << "findcandidate time: " << end5 - start5 << endl;
	...
	for (int i = 0; i < g_cnt; i++) 
	{
		...
		{	
			...
			for (int k = j + 1; k < r_cnt; k++) 
			{
				...
				if ( (vertical_cnt >=1 && diagonal_cnt1>=1) || diagonal_cnt2 ==2)
				{
					cout << "square found" << endl << endl;
					break;
				}
			}
			...
			if (vertical_cnt >= 1 && diagonal_cnt1 >= 1) 
			{
				...
				detected = 1;
				cout << endl << endl << endl << "v1d1"<<endl<< endl;
			}
